...
@pytest.mark.parametrize(
    "matched, idle, busy, to_remove, max_create, max_total, extra, expected_create",
    [
        ...
        (
            ...
            1,
            1,
        ),  # 1 idle VM removed by TTL, should be replaced
        ...
        (
            ...
            1,
            0,
        ),  # Too many idle, should downscale only
    ],
)
def test_decide_scaling(
    matched, idle, busy, to_remove, max_create, max_total, extra, expected_create
):
    remove_copy = list(to_remove)  # simulate pass-by-reference
    to_create, projected_vm_count, excess_idle = decide_scaling(
        matched, idle, busy, remove_copy, max_create, max_total, extra
    )
